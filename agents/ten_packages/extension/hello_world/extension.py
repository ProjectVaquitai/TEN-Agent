...
# from ten_ai_base.llm_tool import AsyncLLMToolBaseExtension, LLMToolMetadata, LLMToolResult, LLMToolMetadataParameter
...
class HelloWorldExtension(AsyncLLMToolBaseExtension):
    ...
    async def run_tool(self, ten_env: AsyncTenEnv, name: str, args: dict) -> LLMToolResult:
        if name == "sing":
            await self.return_song(args, ten_env)
            return LLMToolResultLLMResult(
                type="llmresult",
                content = json.dumps("歌就唱到这里啦，你还有什么需要嘛？")
            )
        
    async def return_song(self, args: dict, ten_env: AsyncTenEnv):
