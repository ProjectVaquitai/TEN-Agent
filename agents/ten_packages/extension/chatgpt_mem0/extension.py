...
class ChatGPTMem0Extension(AsyncLLMBaseExtension):
    ...
    async def on_start(self, ten_env: AsyncTenEnv) -> None:
        ...
        try:
            ...
            ten_env.log_info("Successfully initialized memory manager")
        except Exception as err:
            ten_env.log_error(f"Failed to initialize memory manager: {err}")
            self.memory_manager = None  # 确保设置为 None
            # 不要在这里 return，让系统继续运行，只是没有记忆功能
    ...
    async def on_data(self, ten_env: AsyncTenEnv, data: Data) -> None:
        ...
        try:
            ...
            if hasattr(self, 'memory_manager') and self.memory_manager is not None:
                ...
                ten_env.log_info(f"Retrieved memory text: {memory_text}")
            
            prompt = f"User input: {input_text}"
            if memory_text:
                prompt += f"\nPrevious memories: {memory_text}"
            ten_env.log_info(f"Final prompt: {prompt}")
